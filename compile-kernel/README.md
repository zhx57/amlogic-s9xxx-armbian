...
| --------- | ----------- | ----------- |
| -r        | Repository  | Specifies the kernel source code repository for compilation. Three forms are supported: (1) GitHub owner short-form, e.g. `-r unifreq`, parameter format `owner/repo@branch`. The owner name `owner` is required; `/repo` and `@branch` are optional. When only the owner name is specified, the script automatically matches a repository named `linux-*.y` on the `main` branch under that owner. (2) kernel.org alias: `-r kernel.org` (stable tree, branches `linux-X.Y.y`) or `-r torvalds` (mainline tree). (3) Full git URL: `-r https://git.kernel.org/pub/scm/linux/kernel/git/stable/linux.git@linux-X.Y.y` — any git URL reachable by `git clone` works. Default: `unifreq` |
| -k        | Kernel      | Specifies the kernel version, e.g., `-k 5.15.100`. Multiple kernels are separated with `_`, e.g., `-k 5.15.100_5.15.50`. Using `-k all` compiles all mainline kernels, currently equivalent to `-k 5.10.y_5.15.y_6.1.y_6.6.y_6.12.y_6.18.y_7.0.y`. The kernel list is dynamically adjusted based on the maintenance status of the upstream source repository [unifreq](https://github.com/unifreq). |
...
| Parameter                        | Default Value   | Description                            |
|----------------------------------|-----------------|----------------------------------------|
| ${{ env.PACKAGED_OUTPUTTAGS }}   | 6.12.y_6.18.y_7.0.y   | The name of the compiled kernel.       |
